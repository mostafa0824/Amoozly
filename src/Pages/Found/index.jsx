...
export default function Found() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // search course
      const resCourses = await fetchData(
        `courses?filters[$or][0][title][$containsi]=${name}&filters[$or][1][description][$containsi]=${name}&populate=*`
      );

      // search teacher
      const resTeachers = await fetchData(
        `teachers?filters[name][$containsi]=${name}&populate=*`
      );
      ...
      {courses.length === 0 && teachers.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-8 bg-white rounded-lg shadow-md text-center mt-8">
          ...
          <p className="text-gray-500 mb-4">
            متأسفانه دوره یا مدرسی با مشخصات جستجو شده پیدا نشد.
          </p>
          <p className="text-gray-500 mb-6">
            لطفاً عبارت جستجو را تغییر دهید یا از کلمات کلیدی دیگری استفاده کنید.
          </p>
        ...
        <>
          <p className="text-gray-600 mb-6">
            <span className="font-semibold">{courses.length}</span> نتیجه دوره پیدا شد –{" "}
            <span className="font-semibold">{teachers.length}</span> نتیجه مدرس پیدا شد
          </p>
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 md:gap-4">
            {teachers.map((teacher) => (
              <FoundCart key={teacher.id} tech={teacher} />
            ))}
            {courses.map((course) => (
              <FoundCart key={course.id} item={course} />
            ))}
          </div>
        </>
      )}
